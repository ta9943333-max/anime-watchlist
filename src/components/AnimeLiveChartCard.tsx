...
export function AnimeLiveChartCard({
  ...
  if (poster) {
    return (
      <article
        className={`flex h-full flex-col overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--surface)] transition hover:border-[var(--accent)]/40 ${
          accentClassName ?? ""
        }`}
      >
        {coverBlock}
        <div className="flex min-h-0 flex-1 flex-col p-2">
          <div className="mb-1 flex items-start justify-between gap-1">
            <h3 className="line-clamp-2 text-xs font-semibold leading-snug text-[var(--accent)]">
              {anime.title}
            </h3>
            {headerRight}
          </div>
          {actions ? <div className="mt-auto space-y-1 pt-1">{actions}</div> : null}
          {footerExtra ? (
  ...
  return (
    <article
      className={`flex h-full flex-col overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--surface)] shadow-sm transition hover:border-[var(--accent)]/30 ${
        accentClassName ?? ""
      }`}
    >
      <div className="border-b border-[var(--border)] px-3 py-2.5">
        <div className="flex items-start justify-between gap-2">
          <h3 className="line-clamp-2 text-sm font-bold leading-snug text-[var(--accent)]">
            {anime.title}
          </h3>
          {headerRight ? (
        ...
        {anime.genres.length > 0 && (
          <p className="mt-1 line-clamp-1 text-[11px] leading-relaxed text-[var(--text-dim)]">
